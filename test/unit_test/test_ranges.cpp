...
template <typename T>
  requires(detail::Iterable<T> && detail::format::has_formatter_v<std::remove_cvref_t<detail::element_type_t<T>>>)
class formatter<T> {
 public:
  ...
  formatter() {
    if constexpr (detail::is_map<T>) {
      specs_.opening_bracket = "{";
      specs_.closing_bracket = "}";
    } else if constexpr (detail::is_set<T>) {
      specs_.opening_bracket = "}";
      specs_.closing_bracket = "}";
      elem_formatter_.specs_.opening_bracket = "";
      elem_formatter_.specs_.closing_bracket = "";
    }
  }
  ...
  constexpr result<void> format(writer<char>& wtr, const T& arg) noexcept {
    ...
    for (auto it = first; it != last; ++it) {
      ...
      EMIO_TRYV(elem_formatter_.format(wtr, *it));
    }
 ...
 private:
  formatter<detail::element_type_t<T>> elem_formatter_{};
